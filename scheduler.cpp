...
#define PORT "34388" // the port client will be connecting to
#define BACKLOG 10 // how many pending  connections queue will hold
#define HOS_A "30388"//the port number of hospital A
#define HOS_B "31388"//the port number of hospital B
#define HOS_C "32388"//the port number of hostpial C
#define MYPORT "33388" // the port number of scheduler UDP
...
//    initialize UDP socket information of scheduler, souce code: Beej's Guide to Soket Programming
    ...
    while(1) {
        ...
        inet_ntop(their_addr_TCP.ss_family, get_in_addr((struct sockaddr *)&their_addr_TCP), dst, sizeof dst);
        printf("The Scheduler has received client at location ​%s from the client using TCP over port 34388\n",buf_TCP);
...
//        receive score and distance from hospitals
        ...
        for(int i=1;i<repeat;i++){
            ...
            switch (portnum) {
                case 30388:
                    printScrDis(30388, score, distance);
                    scoreA=score;
                    distanceA=distance;
                    break;
                case 31388:
                    printScrDis(31388, score, distance);
                    scoreB=score;
                    distanceB=distance;
                    break;
                case 32388:
                    printScrDis(32388, score, distance);
                    scoreC=score;
                    distanceC=distance;
                    break;
            }
        ...
        //send updated occupation and availability to corresponding hospital
         ...
         switch (decision) {
             case 1:
                 ...
                 printf("The Scheduler has sent the result to Hospital A using UDP over port 33388.\n");
                 break;
             case 2:
                 ...
                 printf("The Scheduler has sent the result to Hospital B using UDP over port 33388.\n");
                 break;
             case 3:
                 ...
                 printf("The Scheduler has sent the result to Hospital C using UDP over port 33388.\n");
                 break;
         }
